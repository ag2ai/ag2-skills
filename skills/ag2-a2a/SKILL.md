...
Build schemes with the factory helpers in `ag2.a2a.security`, group them into `require(...)` requirements, and pass `security=[...]` to `build_card`. The card's `security_schemes` are auto-derived from the schemes you reference — no duplicate declarations.

```python
from ag2.a2a import build_card
from ag2.a2a.security import api_key_scheme, bearer_scheme, require
...
**Client side** (consuming AG2 agent), register a webhook for a task via the helpers in `ag2.a2a.push`:

```python
from ag2.a2a import A2AConfig
from ag2.a2a.push import A2APushConfig, create_push_notification_config
...
Companion helpers: `get_push_notification_config`, `list_push_notification_configs`, `delete_push_notification_config`. Task inspection helpers live in `ag2.a2a.tasks`: `get_task`, `list_tasks` (returns a `ListedTasks` with pagination metadata), `cancel_task`.
...
`ag2.a2a.testing` ships ASGI helpers that dispatch directly into the server app via `httpx.ASGITransport` — no socket, no port, no SSE proxy. Plug the factory into `A2AConfig.httpx_client_factory` and do a full round trip:
...
from ag2.testing import TestConfig  # mock the served agent's LLM
...
```python
from ag2.a2a.testing import make_test_rest_client_factory
...
- **Missing `a2a` extra** — `pip install "ag2[a2a]"`; without it `from ag2.a2a import A2AServer` raises a "missing additional dependency" error. gRPC additionally needs the `a2a-sdk[grpc]` extra.
...
- `ag2/a2a/server.py` — `A2AServer`, the `build_jsonrpc` / `build_rest` / `build_grpc` builders.
- `ag2/a2a/card.py` — `build_card` and skill/security/interface resolution.
- `ag2/a2a/config.py` — `A2AConfig`, `from_card`, all client options.
- `ag2/a2a/security.py` — scheme factories and `require`.
- `ag2/a2a/push.py` / `tasks.py` — client-side push + task helpers.
- `ag2/a2a/testing.py` — in-process test factories.
- `ag2/a2a/events.py` — `A2AEvent` family for observing wire events on the AG2 stream.
